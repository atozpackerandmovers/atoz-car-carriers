...
function installFloatingActions() {
  ...
  const phoneHref = "tel:+919338888550";
  const message = encodeURIComponent(
    "Hello A TO Z Car Carriers, I need vehicle transport service."
  );
...
function showThankYou(enquiryId, deliveryStatus) {
  ...
  const thankYouPath = window.location.pathname.includes("/odisha/")
    ? "../thank-you.html"
    : "thank-you.html";
...
function prepareNativeFormFallback(form, enquiryId) {
  ...
  const basePath = window.location.pathname.includes("/odisha/")
    ? "../thank-you.html"
    : "thank-you.html";
...
async function handleFormSubmit(event) {
  ...
  if (statusEl) statusEl.textContent = "Submitting your enquiry...";
  ...
  if (statusEl) {
    statusEl.textContent =
      `AJAX submit failed, trying direct Formspree submit now. Enquiry ID: ${enquiryId}`;
...
estimateButton?.addEventListener("click", () => {
  ...
  if (result) {
    result.innerHTML = `Rs ${low.toLocaleString("en-IN")} - Rs ${high.toLocaleString("en-IN")}<small>Indicative range only. Final quote depends on exact pickup, route, vehicle condition, insurance and carrier availability.</small>`;
  }
});
